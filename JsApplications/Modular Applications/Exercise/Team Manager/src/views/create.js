...
export async function createPage(ctx) {
  ...
  async function onSubmit(event) {
    ...
    Array.from(event.target.querySelectorAll("input")).forEach(i => i.disabled = true)
    ...
    try{
      ...
      if(logoUrl == ""){
        throw new Error("Teamlogo is required!") 
      }
      ...
      Array.from(event.target.querySelectorAll("input")).forEach(i => i.disabled = false)
    }
  }
}